...
\begin{proof}
	...
	\newline
	Da $X$ Hausdorffsch ist, existieren offene Teilmengen $U,V \subseteq X$, mit $U \cap V = \emptyset$ und $x \in U, x^\prime \in V$. Dann existieren $n_0, n_0^\prime \in \N$ mit $x_n \in U, x_m \in V \text{ für alle } n \in \N_{\geq n_0}, m \in \N_{\geq n_0^\prime}$. Dann gilt also für alle $k \geq \max\{n_0,n_0^\prime\} \colon x_k \in U \cap V$. $\lightning$
\end{proof}
...
\begin{proof}
[\underline{z.z.:} $\forall x,y \in X \exists U_x, U_y \subseteq X \colon U_x \cap U_y = \emptyset$.]
Seien $x,y \in X$. Wähle $U_x := B_\frac{d(x,y)}{3}(x), U_y := B_\frac{d(x,y)}{3}(y)$.
\newline
...
\begin{enumerate}
	...
	\item \underline{1-dimensionale glatte Mannigfaltigkeiten} (TODO: Bild 1)
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item $(a,b)$ ist lokal homöomorph zu $\R$, \underline{denn:} Es gilt: $id \colon (a,b) \mapsto (a,b) \subseteq \R$ ist ein Homöomorphismus einer offenen Menge in eine offene Teilmenge von $\R$. Somit ist $\left((a,b),id\right)$ eine (\underline{globale})\footnote{Global, da für die ganze Mannigfaltigkeit gleich.} Karte.
				\item Für den Kartenwechsel gilt: $id \circ id^{-1} \colon (a,b) \rightarrow (a,b), x \mapsto x$, ist eine glatte Abbildung.
			\end{itemize}
	...
	\item Es ist $\R^n$ eine $n$-dimensionale glatte Mannigfaltigkeit, \underline{denn:}
		\begin{itemize}
			...
			\item $(\R^n, id)$ ist eine globale Karte.
		\end{itemize}
\end{enumerate}
...
\begin{proof}
	...
	$$\varphi \circ \varphi^{-1} = id \colon \varphi(U) \rightarrow \varphi(U)$$ und dieser ist natürlich glatt.
\end{proof}
...
\begin{proof}
	Es sei $M$ eine $C^k$-Mannigfaltigkeit, $N \subseteq_{offen} M$.
	\begin{itemize}
